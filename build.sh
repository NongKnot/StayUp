...
# Sparkle.framework — auto-updater. Vendored at Vendor/Sparkle.framework
# (gitignored, regenerable from https://github.com/sparkle-project/Sparkle
# releases). Embedded into the bundle at @executable_path/../Frameworks/
# so the rpath set in swiftc resolves at runtime.
...
# Source generated by tools/make-icon.sh from the Duck SVG.
if [ -f Resources/AppIcon.icns ]; then
    cp Resources/AppIcon.icns "${BUNDLE}/Contents/Resources/"
else
    echo "⚠  Resources/AppIcon.icns missing — run: bash tools/make-icon.sh"
fi

# Activity Source hook script — ActivitySourceHookInstaller copies this out of the bundle
# to ~/.stayup/bin/ at install time (see docs/activity-source-contract.md).
...
# without --timestamp so this works offline; release.sh re-signs with
...
if [ -n "$DEV_ID" ] && security find-identity -v -p codesigning 2>/dev/null | grep -q "$DEV_ID"; then
    ...
    # Sign them before sealing the app, otherwise the outer bundle verifies
    # but the deep release-readiness check fails.
    codesign --force \
        --sign "$DEV_ID" \
        "${BUNDLE}/Contents/Resources/stayup-source-hook.sh" >/dev/null
    codesign --force \
